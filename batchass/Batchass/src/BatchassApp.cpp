/*
 Copyright (c) 2013-2025, Bruce Lane - All rights reserved.
 ...
 */
 ...
 /*
 TODO
 preview fbo on drop or at a moment
 inputimage.fs no works
 */
...
BatchassApp::BatchassApp() : mSpoutOut("Batchass", app::getWindowSize())
{
	...
	mVDSessionFacade = VDSessionFacade::createVDSession(mVDSettings, mVDAnimation, mVDUniforms, mVDMix)
		...
		//->setupMidi()
		->addOSCObserver(mVDSettings->mOSCDestinationHost, mVDSettings->mOSCDestinationPort)
		->setUniformValue(mVDUniforms->IBPM, 70.0f) // was 120.0f
		->setUniformValue(mVDUniforms->IMOUSEX, 0.27710f) // sos only
		->setUniformValue(mVDUniforms->IMOUSEY, 0.5648f) // sos only
		//->setUniformValue(mVDUniforms->IEXPOSURE, 3.965f) // mtrlvr sos only 
...
void BatchassApp::toggleCursorVisibility(bool visible)
{
	if (visible)
	{
		showCursor();
...
void BatchassApp::draw()
{
	...
	if (mFadeInDelay) {
		...
		// 20211227 textures needs updating?
	...
	if (mVDSessionFacade->showUI()) {
		mVDUI->Run("UI", (float)getAverageFps());
		if (mVDUI->isReady()) {
		}
	}
...
CINDER_APP(BatchassApp, RendererGl(RendererGl::Options().msaa(8)), prepareSettings)
