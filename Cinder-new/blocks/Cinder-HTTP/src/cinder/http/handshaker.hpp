...
private:
	...
	{
		if( !ec )
			detail::http_post( mSession->socket, std::bind( &SessionType::onHandshake, mSession, ec ) );
		else
			detail::http_post( mSession->socket, std::bind( &SessionType::onError, mSession, ec ) );
	}
